...
function switchView(viewName) {
  ...
  if (viewName === 'template') {
    initPTMPDefaults();
    initKonfirmasiPenerimaDefaults();
  }
...
function switchSubTemplate(subName) {
  ...
  const tabKp = document.getElementById('tab-konfirmasi-penerima');
  ...
  const secKp = document.getElementById('subtemplate-konfirmasi-penerima');
  ...
  if (tabKp) tabKp.classList.toggle('active', subName === 'konfirmasi_penerima');
  ...
  if (secKp) secKp.classList.toggle('hidden', subName !== 'konfirmasi_penerima');
  ...
  if (subName === 'ptmp') {
    ...
    initNoWADefaults();
  } else if (subName === 'konfirmasi_penerima') {
    initKonfirmasiPenerimaDefaults();
  }
}
...
// --------------------------------------------------------------------------
// 13.5. TEMPLATE #5: KONFIRMASI PENERIMA
// --------------------------------------------------------------------------
function initKonfirmasiPenerimaDefaults() {
  updateKonfirmasiPenerimaPreview();
}

function generateKonfirmasiPenerimaMessage() {
  const resi = document.getElementById('kp-resi')?.value.trim() || '...';
  const customer = document.getElementById('kp-customer-name')?.value.trim() || '...';
  const isi = document.getElementById('kp-isi-paket')?.value.trim() || '...';
  
  const rawCod = document.getElementById('kp-cod-price')?.value.trim() || '';
  let cleanCod = rawCod.replace(/^rp\.?\s*/i, '').replace(/,-$/, '').trim();
  if (cleanCod && /^\d+$/.test(cleanCod)) {
    cleanCod = parseInt(cleanCod, 10).toLocaleString('id-ID');
  }
  const codText = cleanCod || '.....';

  return (
    `Halo kak, kami dari J&T Express 😊\n\n` +
    `Mau konfirmasi terkait paket dengan nomor resi ${resi}, a.n. ${customer}, dengan isi paket *${isi}*.\n` +
    `Dengan Harga COD Rp *Rp ${codText} ,-*\n\n` +
    `Apakah paketnya sudah diterima, kak?\n\n` +
    `Kalau sudah diterima, boleh dibantu informasikan *paket diterima kapan* dan untuk *pembayaran COD-nya dilakukan melalui apa* ya kak?\n\n` +
    `Mohon konfirmasinya yaa kak. \n` +
    `Terima kasih 🙏`
  );
}

function updateKonfirmasiPenerimaPreview(showFeedback = false) {
  const textarea = document.getElementById('kp-result-text');
  if (!textarea) return;

  const msg = generateKonfirmasiPenerimaMessage();
  textarea.value = msg;

  if (showFeedback) {
    showToast('⚡ Pesan Konfirmasi Penerima berhasil diperbarui!', 'success');
  }
}

function copyKonfirmasiPenerimaText() {
  const message = generateKonfirmasiPenerimaMessage();
  navigator.clipboard.writeText(message).then(() => {
    showToast('📋 Pesan Konfirmasi Penerima berhasil disalin!', 'success');
  }).catch(() => {
    const textarea = document.getElementById('kp-result-text');
    if (textarea) {
      textarea.select();
      document.execCommand('copy');
      showToast('📋 Pesan Konfirmasi Penerima disalin!', 'success');
    }
  });
}

function shareKonfirmasiPenerimaWhatsApp() {
  const message = generateKonfirmasiPenerimaMessage();
  const phoneInput = document.getElementById('kp-customer-phone')?.value.trim() || '';
...
function extractCleanBranchName(val) {
  if (!val) return '';
  const str = String(val).trim();
  const lower = str.toLowerCase();
  // Header cells to skip
  if (['dp delivery', 'dp_delivery', 'drop point', 'nama dp', 'nama drop point', 'dp', 'agent', 'tanggal', 'cabang', 'branch', 'kode sprinter', 'sprinter delivery'].includes(lower)) {
    return '';
  }
  // Skip if timestamp/date or waybill number
  if (str.includes('202') && str.includes(':')) return '';
  if (/^\d+$/.test(str) && str.length > 5) return '';
  
  // Replace underscores with space and uppercase
...
function parseDeliveryRows(rows) {
  ...
  for (let r = 0; r < Math.min(10, rows.length); r++) {
    ...
    for (let c = 0; c < row.length; c++) {
      ...
      if ((cell.includes('dp delivery') || cell.includes('nama dp') || cell.includes('drop point') || cell === 'dp' || cell === 'cabang' || cell.includes('branch')) && colIndexMap.dp === -1) {
        colIndexMap.dp = c;
      }
  ...
  if (isDetailFormat) {
    ...
    for (let r = startLine; r < rows.length; r++) {
      ...
      if (dpVal && !detectedBranch) {
        const clean = extractCleanBranchName(dpVal);
        if (clean) detectedBranch = clean;
      }
...
function handleManualBranchEdit(newBranch) {
  const clean = (newBranch || '').trim().toUpperCase();
  if (clean) {
    if (originalDeliveryDataState) {
      originalDeliveryDataState.branch = clean;
    }
    if (generatedDeliveryReportData) {
      generatedDeliveryReportData.branch = clean;
    }
    showToast(`🏢 Nama cabang disesuaikan: ${clean}`, 'info');
  }
}
