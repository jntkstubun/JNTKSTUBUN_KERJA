...
function switchView(viewName) {
  ...
  const views = ['dashboard', 'merge', 'upload', 'template'];
  views.forEach(v => {
...
function syncBranchName(source) {
  ...
  if (source === 'ptmp' && ptmpBranch && returanBranch) {
    returanBranch.value = ptmpBranch.value;
  } else if (source === 'returan' && ptmpBranch && returanBranch) {
    ptmpBranch.value = returanBranch.value;
  }
  saveUserPreferences();
}
...
function initPTMPDefaults() {
  loadPreferences();
  const dateInput = document.getElementById('ptmp-date');
  const timeInput = document.getElementById('ptmp-time');
...
function generatePTMPMessage() {
  ...
  const wrongAddress = document.getElementById('ptmp-wrong-address')?.value.trim() || 'Gg. Melati No. 10';
  const dateVal = document.getElementById('ptmp-date')?.value || '';
  const timeVal = document.getElementById('ptmp-time')?.value || '';
  const tzVal = document.getElementById('ptmp-tz')?.value || 'WITA';

  const timeFormatted = formatSmartDateTime(dateVal, timeVal, tzVal);

  return (
    `Permisi Kak, saya dari J&T Express ${branchName}.\n\n` +
    `Saya ${role} ${staffName}, ingin mengonfirmasi terkait paket Kakak atas nama ${customerName} dengan nomor resi ${resi}.\n\n` +
    `Sebelumnya, paket sempat salah diantarkan ke ${wrongAddress}. Paket tersebut kemudian telah diantarkan kembali sesuai dengan alamat yang benar pada ${timeFormatted}.\n\n` +
...
function switchSubTemplate(subName) {
  ...
  if (subName === 'ptmp') {
    if (tabPtmp) tabPtmp.classList.add('active');
    if (tabReturan) tabReturan.classList.remove('active');
    if (secPtmp) secPtmp.classList.remove('hidden');
    if (secReturan) secReturan.classList.add('hidden');
    initPTMPDefaults();
  } else if (subName === 'returan') {
    if (tabPtmp) tabPtmp.classList.remove('active');
    if (tabReturan) tabReturan.classList.add('active');
    if (secPtmp) secPtmp.classList.add('hidden');
    if (secReturan) secReturan.classList.remove('hidden');
    initReturanDefaults();
  } else {
    showToast('Template ini sedang dalam pengembangan.', 'info');
  }
}
...
function startHeaderLiveClock() {
  function updateClock() {
    ...
    clockEl.textContent = `${dayName}, ${day} ${monthName} ${year} • ${hh}:${mm}:${ss} WITA`;
  }
...
function applyThemeMode(theme) {
  ...
  if (theme === 'dark') {
    ...
    if (bottomIcon) bottomIcon.className = 'fa-solid fa-sun';
    if (bottomLabel) bottomLabel.textContent = 'Terang';
  } else {
    document.body.classList.remove('dark-mode');
    if (btnIcon) btnIcon.className = 'fa-solid fa-moon';
    if (btnText) btnText.textContent = 'Mode Gelap';
    if (bottomIcon) bottomIcon.className = 'fa-solid fa-moon';
    if (bottomLabel) bottomLabel.textContent = 'Gelap';
  }
}
